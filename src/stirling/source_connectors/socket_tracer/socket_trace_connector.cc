...
Status SocketTraceConnector::InitImpl(bpf_tools::BCCWrapper* bcc) {
  DCHECK( bcc != nullptr );
  DCHECK( bcc_ == nullptr);
  bcc_ = bcc;
  CheckDebugFlags();
  ...
  uprobe_mgr_.Init(bcc_, FLAGS_stirling_disable_golang_tls_tracing, protocol_transfer_specs_[kProtocolHTTP2].enabled, FLAGS_stirling_disable_self_tracing);
...
void SocketTraceConnector::TransferDataImpl(ConnectorContext* ctx) {
  ...
  if (sampling_freq_mgr_.count() % (kDebugDumpPeriod / kSamplingPeriod) == 0) {
    if (debug_level_ >= 1) {
      ...
      LOG(INFO) << "BPF map info: " << BPFMapsInfo(bcc_);
    }
  }
...
Status SocketTraceConnector::UpdateBPFProtocolTraceRole(traffic_protocol_t protocol,
                                                        uint64_t role_mask) {
  auto control_map = WrappedBCCPerCPUArrayTable<uint64_t>::Create(bcc_, kControlMapName);
...
Status SocketTraceConnector::TestOnlySetTargetPID() {
  ...
  auto control_map = WrappedBCCPerCPUArrayTable<int64_t>::Create(bcc_, kControlValuesArrayName);
