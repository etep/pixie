...
Status PerfProfileConnector::InitImpl(bpf_tools::BCCWrapper* bcc) {
  DCHECK(bcc != nullptr);
  DCHECK(bcc_ == nullptr);
  bcc_ = bcc;
  ...
  PX_RETURN_IF_ERROR(bcc_->OpenPerfBuffers(perf_buffer_specs, this));

  stack_traces_a_ = WrappedBCCStackTable::Create(bcc_, "stack_traces_a");
  stack_traces_b_ = WrappedBCCStackTable::Create(bcc_, "stack_traces_b");

  profiler_state_ = WrappedBCCArrayTable<uint64_t>::Create(bcc_, "profiler_state");
