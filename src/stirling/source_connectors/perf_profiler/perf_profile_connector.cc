...
Status PerfProfileConnector::InitImpl() {
  ...
  const auto perf_buffer_specs = MakeArray<bpf_tools::PerfBufferSpec>(
      {{"histogram_a", HandleHistoEvent, HandleHistoLoss, perf_buffer_size,
        bpf_tools::PerfBufferSizeCategory::kUncategorized, this},
  ...
  stack_traces_a_ = bpf_tools::WrappedBCCStackTable::Create(this, "stack_traces_a");
  stack_traces_b_ = bpf_tools::WrappedBCCStackTable::Create(this, "stack_traces_b");
...
void PerfProfileConnector::ProcessBPFStackTraces(ConnectorContext* ctx, DataTable* data_table) {
  ...
  CreateRecords(stack_traces->RawPtr(), ctx, data_table);

  uint64_t num_stack_traces_sampled = 0;
  auto num_stack_traces_sampled_or_status = profiler_state_->GetValue(sample_count_idx);
  if (num_stack_traces_sampled_or_status.ok()) {
    num_stack_traces_sampled = num_stack_traces_sampled_or_status.ConsumeValueOrDie();
  }
...
void PerfProfileConnector::CheckProfilerState(const uint64_t num_stack_traces) {
  ...
  auto error_code_or_status = profiler_state_->GetValue(kErrorStatusIdx);
  if (error_code_or_status.ok()) {
    error_code = error_code_or_status.ConsumeValueOrDie();
  }
