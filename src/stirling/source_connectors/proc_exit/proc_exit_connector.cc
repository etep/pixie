...
Status ProcExitConnector::InitImpl() {
  ...
  if (offset_opt.has_value()) {
    ...
    auto control_values_table_handle = bpf_tools::WrappedBCCPerCPUArrayTable<uint64_t>::Create(
        this, kProcExitControlValuesArrayName);
    PX_RETURN_IF_ERROR(control_values_table_handle->SetValues(TASK_STRUCT_EXIT_CODE_OFFSET_INDEX,
  ...
  const auto kPerfBufferSpecs = MakeArray<bpf_tools::PerfBufferSpec>({
      {"proc_exit_events", HandleProcExitEvent, HandleProcExitEventLoss, kPerfBufferPerCPUSizeBytes,
       bpf_tools::PerfBufferSizeCategory::kControl, this},
  });
