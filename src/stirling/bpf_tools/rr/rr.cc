...
void RRSingleton::ReplayPerfBufferEvents(const PerfBufferSpec& perf_buffer_spec) {
  if (PlabackComplete()) {
    LOG_FIRST_N(WARNING, 1) << "RRSingleton::ReplayPerfBufferEvents(), plaback complete.";
    return;
  }
...
Status RRSingleton::ReplayArrayGetValue(const std::string& name, const int32_t idx, void* value,
  ...
  // LOG(WARNING) << "RRSingleton::ReplayArrayGetValue().";
...
void RRSingleton::SetReplaying() {
  ECHECK(!recording_);
  replaying_ = true;
}

Status RRSingleton::OpenReplayProtobuf(const std::string& replay_events_pb_file_path) {
  LOG(WARNING) << "RRSingleton::OpenReplayProtobuf(), replay_events_pb_file_path: "
               << replay_events_pb_file_path << ".";
  ...
  LOG(WARNING) << "RRSingleton::OpenReplayProtobuf(), events_proto_.size(): "
               << events_proto_.event_size() << ".";
  return Status::OK();
}
