...
class SocketTracerReplayer : public UnitConnector<SocketTraceConnector, bpf_tools::ReplayingBCCWrapperImpl> {};
...
Status StirlingWrapperCallback(uint64_t table_id, px::types::TabletID, std::unique_ptr<px::types::ColumnWrapperRecordBatch> record_batch) {
...
Status RunSocketTracer() {
  // Open the replay protobuf file.
  auto& rr = px::stirling::bpf_tools::RRSingleton::GetInstance();
  PX_RETURN_IF_ERROR(rr.OpenReplayProtobuf(FLAGS_events_pb_file));
  ...
  for (const auto& schema : underyling_source->table_schemas()) {
    LOG(INFO) << absl::Substitute("Adding info class: [$0/$1]", underyling_source->name(), schema.name());
